...
if __name__ == '__main__':
    ...
    print(np.corrcoef(regYHat, testMat[:, 1], rowvar=0)[0, 1])

    modelTree = createTree(trainMat, modelLeaf, modelErr, ops=(1, 20))
    modelYHat = createForecast(modelTree, testMat[:, 0], modelTreeEval)
    print(np.corrcoef(modelYHat, testMat[:, 1], rowvar=0)[0, 1])


    ws, X, Y = linearSolve(trainMat)
    lrYHat = np.zeros((Y.shape[0], 1))
    for i in range(testMat.shape[0]):
        lrYHat[i] = testMat[i, 0] * ws[1, 0] + ws[0, 0]

    print(np.corrcoef(lrYHat, testMat[:, 1], rowvar=0)[0, 1])
